...
class OpcodeResult:
    ...
    def __init__(
            ...
            env_source: tuple[int, int] | None = None,
    ):
        """
        Args:
            ...
            env_source: Set only by GetEnvironment/GetParentEnvironment,
                to `(effective_depth, None)`, and by LoadFromEnvironment,
                to `(effective_depth, slot)` - the (depth, slot) pair
                PrivateNameTable/ClassEnvironmentTable resolve a real
                name from, carried forward through any `Mov` of the same
                register (see Mov.py) so a private-field/class-reference
                opcode several instructions later can still recover it
                from wherever the register it was handed currently
                traces back to. `None` for every other opcode. See
                EnvironmentOriginTable's own docstring for what
                "effective depth" means here.
