function function_12825(param0, param1, param2, param3, param4) {
    ...
    // LOOP → START (while)
    while (!param1.slice(param1).length) {
        // ──────────────── Block 1 ──────────────── 
        // CODE → <Mov>: <Reg8: 1, Reg8: 10>
        // USED → r1 = param1.slice(param1);
        // CODE → <GetByIdShort>: <Reg8: 1, Reg8: 1, UInt8: 2, string_id: 139>  # String: 'length' (Identifier)
        // USED → r1 = param1.slice(param1).length;
        // CODE → <JmpFalse>: <Addr8: 81, Reg8: 1>  # Address: 00000080
        if (!param1.slice(param1).length) goto label_128;
        // ──────────────── Block 2 ──────────────── 
        // CODE → <Mov>: <Reg8: 11, Reg8: 10>
        // USED → r11 = param1.slice(param1);
        // CODE → <GetById>: <Reg8: 1, Reg8: 11, UInt8: 3, string_id: 16799>  # String: 'pop' (Identifier)
        // USED → r1 = param1.slice(param1).pop;
        // CODE → <Call1>: <Reg8: 11, Reg8: 1, Reg8: 11>
        // USED → r11 = param1.slice(param1).pop(param1.slice(param1));
        // CODE → <Mov>: <Reg8: 1, Reg8: 7>
        // USED → r1 = param2;
        // CODE → <Call2>: <Reg8: 1, Reg8: 11, Reg8: 6, Reg8: 1>
        // USED → r1 = param1.slice(param1).pop(param1.slice(param1))(undefined, param2);
        // CODE → <StrictEq>: <Reg8: 0, Reg8: 1, Reg8: 5>
        // USED → r0 = param1.slice(param1).pop(param1.slice(param1))(undefined, param2) === false;
        // CODE → <Jmp>: <Addr8: 47>  # Address: 0000007a
        goto label_122;
        // LOOP → START (while)
        while (!(param1.slice(param1).pop(param1.slice(param1))(undefined, param2) === false)) {
            // ──────────────── Block 4 ──────────────── 
            // CODE → <Mov>: <Reg8: 1, Reg8: 0>
            // USED → r1 = param1.slice(param1).pop(param1.slice(param1))(undefined, param2) === false;
            // CODE → <JmpFalse>: <Addr8: -86, Reg8: 1>  # Address: 00000027
            if (!(param1.slice(param1).pop(param1.slice(param1))(undefined, param2) === false)) goto label_39;
            // ──────────────── Block 3 ──────────────── 
            // CODE → <Catch>: <Reg8: 12>
            ...
            r1 = param4.error(param4, caughtException);
        }
        // LOOP → END
    }
    // LOOP → END
