function processColorsInProps(param0, param1) {
    ...
    while (HermesPropertyIterator(r10).next() === undefined) {
        ...
        // USED → r13 = HermesPropertyIterator(r10).next()
        // CODE → <Mov>: <Reg8: 7, Reg8: 13>
        // USED → r7 = HermesPropertyIterator(r10).next()
        ...
        // USED → r11 = getEnvironment(1)[17].includes(getEnvironment(1)[17], HermesPropertyIterator(r10).next())
        // CODE → <JmpTrue>: <Addr8: 98, Reg8: 11>  # Address: 0000009e
        if (getEnvironment(1)[17].includes(getEnvironment(1)[17], HermesPropertyIterator(r10).next())) { /* jump to label_158 */ }
        // ──────────────── Block 3 ──────────────── 
        // CODE → <LoadFromEnvironment>: <Reg8: 12, Reg8: 1, UInt8: 18>
        // USED → r12 = getEnvironment(1)[18]
        // CODE → <Mov>: <Reg8: 11, Reg8: 7>
        // USED → r11 = HermesPropertyIterator(r10).next()
        // CODE → <GetByVal>: <Reg8: 11, Reg8: 12, Reg8: 11>
        // USED → r11 = getEnvironment(1)[18][HermesPropertyIterator(r10).next()]
        // CODE → <JmpFalse>: <Addr8: -46, Reg8: 11>  # Address: 0000001c
        if (!getEnvironment(1)[18][HermesPropertyIterator(r10).next()]) { /* jump to label_28 */ }
        // LOOP → START (while)
        while (true) {
            // ──────────────── Block 10 ──────────────── 
            // CODE → <Mov>: <Reg8: 12, Reg8: 7>
            // USED → r12 = HermesPropertyIterator(r10).next()
            // CODE → <LoadFromEnvironment>: <Reg8: 13, Reg8: 1, UInt8: 36>
            // USED → r13 = getEnvironment(1)[36]
            // CODE → <GetByVal>: <Reg8: 11, Reg8: 10, Reg8: 12>
            // USED → r11 = r10[HermesPropertyIterator(r10).next()]
            // CODE → <Call2>: <Reg8: 11, Reg8: 13, Reg8: 0, Reg8: 11>
            // USED → r11 = getEnvironment(1)[36](undefined, r10[HermesPropertyIterator(r10).next()])
            // CODE → <PutByVal>: <Reg8: 10, Reg8: 12, Reg8: 11>
            r10[HermesPropertyIterator(r10).next()] = getEnvironment(1)[36](undefined, r10[HermesPropertyIterator(r10).next()]);
    ...
    // USED → r13 = r10[HermesPropertyIterator(r10).next()]
    // CODE → <Mov>: <Reg8: 11, Reg8: 13>
    // USED → r11 = r10[HermesPropertyIterator(r10).next()]
    // CODE → <IteratorBegin>: <Reg8: 12, Reg8: 11>
    // USED → r12 = GetIterator(r10[HermesPropertyIterator(r10).next()])
    // LOOP → START (while)
    while (GetIterator(r10[HermesPropertyIterator(r10).next()]) === undefined) {
        // ──────────────── Block 5 ──────────────── 
        // CODE → <IteratorNext>: <Reg8: 14, Reg8: 12, Reg8: 11>
        // USED → r14 = GetIterator(r10[HermesPropertyIterator(r10).next()]).next()
        // CODE → <Mov>: <Reg8: 13, Reg8: 12>
        // USED → r13 = GetIterator(r10[HermesPropertyIterator(r10).next()])
        // CODE → <JStrictEqual>: <Addr8: -69, Reg8: 13, Reg8: 0>  # Address: 0000001c
        if (GetIterator(r10[HermesPropertyIterator(r10).next()]) === undefined) { /* jump to label_28 */ }
        // ──────────────── Block 6 ──────────────── 
        // CODE → <Mov>: <Reg8: 8, Reg8: 14>
        // USED → r8 = GetIterator(r10[HermesPropertyIterator(r10).next()]).next()
        ...
        // USED → r13 = getEnvironment(1)[18][HermesPropertyIterator(r10).next()]
        // CODE → <Mov>: <Reg8: 9, Reg8: 13>
        // USED → r9 = getEnvironment(1)[18][HermesPropertyIterator(r10).next()]
        // CODE → <GetByVal>: <Reg8: 13, Reg8: 14, Reg8: 13>
        // USED → r13 = GetIterator(r10[HermesPropertyIterator(r10).next()]).next()[getEnvironment(1)[18][HermesPropertyIterator(r10).next()]]
        // CODE → <JStrictEqual>: <Addr8: 27, Reg8: 13, Reg8: 0>  # Address: 00000095
        if (GetIterator(r10[HermesPropertyIterator(r10).next()]).next()[getEnvironment(1)[18][HermesPropertyIterator(r10).next()]] === undefined) { /* jump to label_149 */ }
        // ──────────────── Block 7 ──────────────── 
        // CODE → <Mov>: <Reg8: 15, Reg8: 8>
        // USED → r15 = GetIterator(r10[HermesPropertyIterator(r10).next()]).next()
        // CODE → <Mov>: <Reg8: 14, Reg8: 9>
        // USED → r14 = getEnvironment(1)[18][HermesPropertyIterator(r10).next()]
        ...
        // USED → r13 = getEnvironment(1)[36](undefined, GetIterator(r10[HermesPropertyIterator(r10).next()]).next()[getEnvironment(1)[18][HermesPropertyIterator(r10).next()]])
        // CODE → <PutByVal>: <Reg8: 15, Reg8: 14, Reg8: 13>
        GetIterator(r10[HermesPropertyIterator(r10).next()]).next()[getEnvironment(1)[18][HermesPropertyIterator(r10).next()]] = getEnvironment(1)[36](undefined, GetIterator(r10[HermesPropertyIterator(r10).next()]).next()[getEnvironment(1)[18][HermesPropertyIterator(r10).next()]]);
        // ──────────────── Block 8 ──────────────── 
